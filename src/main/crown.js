...
function loadMiceFromUrlOrCookie() {
    var mouseList = getStringListFromURL(window.location.search.match(/mice=([^&]*?)\?catches=/));
	var numCatchList = getStringListFromURL(window.location.search.match(/catches=([^&]*)/));
...
var buildMouselist = function (mouseListText, sortedMLCLength, sortedMLC) {
	for (var l = 0; l < sortedMLCLength; l++) {
            ...
            mouseListText += "<td style=\'font-size: 11px; padding: 10px\'>" + "<p style='font-size: 16px'>" + sortedMLC[l][1].toFixed(2) + "%</p><br>" + sliceMLC + "</td>";
        }
				...
				for (var k=0; k<noPhases; k++) {
					...
					for (var l=0; l<noCheeses; l++) {
						...
						for (var m=0; m<noCharms; m++) {
							...
							var attractionRate = parseFloat(popArray[mouseName][locationName][phaseName][cheeseName][charmName]) / catchesFromSilver;
	...
	for (var i=0; i<sortedLocationLength; i++) {
		...
		if (mouseLocationArray[lpcc]) {
			for (var j=0; j<Object.size(mouseLocationArray[lpcc]); j++) {
				mouseLocationHTML += mouseLocationArray[lpcc][j][0] + " (" + mouseLocationArray[lpcc][j][1].toFixed(2) + "%)<br>";
			}
		}
		else {
