...
function requestForm (form, opts) {
  ...
  return rp.post('http://horntracker.com/backend/new/viewer.php', {
    form: form
  }).then(function (data) {
    return JSON.parse(data)
  }).catch(function (err) {
    debug('error', err)
    if (opts.retry) return Promise.delay(30000).then(requestForm.bind(this, opts))
...
module.exports = function (vars, opts) {
  ...
  return requestForm(serializeVars(vars), opts)
    .then(function (data) {
      ...
      }).sort(function (a, b) { return b.attraction - a.attraction })
    })
}